...
1. **Installation**: Ensure the package is installed:
   ...
   ```
2. **Database URI**: You must have the database connection URI or connection string. This can be:
   - Passed explicitly via `--db-uri` / `-d`
   - Set as `DATABASE_URL` in the shell environment
   - Defined in a local `.env` file in the current working directory (the tool loads `.env` automatically)
...
If `DATABASE_URL` is set in a local `.env` file or your environment, you do not need to provide the `--db-uri` / `-d` argument:
